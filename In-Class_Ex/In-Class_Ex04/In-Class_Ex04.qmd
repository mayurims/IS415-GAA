...
```{r}
# Sf saves the data into dataframe format
...
```{r}
# We put a small distance btwn the points (nsim=1) to avoid duplication
...
```{r}
# Changing it to km, so that it is easier to understand the density.Both maps are identical, its just the range. Make sure the title of the map is clear
