...
**HOW DOO YOU FIND THE LEVELSSSS????????? DRAW DIAGRAM!**
...
Our range for the breakpoints is 37 to 90. Hence, our breakpoints will be

**HOW DO U FIND THE BREAKPOINTSS????**
...
We have plotted p-value less than 0.05. As the p-value represents the probability of observing a clustering. Hence, a significant p-value (i.e., \<0.05) suggests that the observed pattern is unlikely to have occurred by chance and may indicate the presence of a spatial process. Hence, the vaccination rate of the sub-district at that period is significant. Further, when Gi\* value \> 0, it indicates sub-districts with a higher vaccination rate than average. Which can be seen through our maps. Now lets see the number of times each sub-district had a p-value less than 0.05

```{r}
p_value_freq = filter(jakarta_gi_values, p_sim<0.05)
as.data.frame(table(p_value_freq$Sub_District))
```

As we can see there are 128 sub-districts who have a significant vaccination rate (p-value \< 0.05) atleast once during the period of 12 months. However, the following 12 sub-districts -- "BATU AMPAR", "CIPINANG CEMPEDAK", "GLODOK", "KAMPUNG BALI", "KAMPUNG TENGAH", "KEAGUNGAN", "KEBON KACANG", "KEBON MELATI", "MANGGA BESAR", "PETAMBURAN" have a significant p-value throughout the course of 12 months.

# 9.0 Emerging Hot Spot Anlaysis (EHSA)

Emerging Hot Spot Analysis (EHSA) is a spatio-temporal analysis method for revealing and describing how hot spot and cold spot areas evolve over time. Since, we have already built a space-time cube and calculate the Gi\* statistict, we can directly conduct the Mann-Kendall trend test to evaluate 3 sub-districts for a trend. For this analysis, I will be choosing HALIM PERDANA KUSUMAH, KOTA BAMBU SELATAN and KAMAL.
...
***The p-value is 0.086 which is \> 0.05 hence p-value is not significant. This result tells us that here is an overall upward but insignificant trend.***
...
***The p-value is 0.01 which is \< 0.05 hence p-value is significant. This result tells us that here is an overall upward and a significant trend.***
...
***The p-value is 0.0001 which is \< 0.05 hence p-value is significant. This result tells us that here is an overall downward and significant trend.***
...
The maps shows the largest number of oscillating hotspots which are located evenly in Jakarta. This is followed by Spordiac coldspot, which is spread out in Jakarta but less than oscillating hotspots, as we don't see that many spordiac coldspots near the borders of Jakarta. This is lastly followed by Oscilating coldspot which appear to be more around the border and in the central of Jakarta. Lastly, there are areas with no pattern detected, these are mainly located in the central area. Further, the sub-districts with p-value \> 0.05 are in grey colour, as they are insignificant.

# 10.0 References

Thank your Prof. Kim for all the resources and insightful Hands-On and Take-Home Excercises. Special thanks go to the Senior Sample Submissions that Prof. Kam recommended!
