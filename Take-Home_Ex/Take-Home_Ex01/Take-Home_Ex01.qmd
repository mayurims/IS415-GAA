...
In the code chunk below, [`st_knn()`](https://sfdep.josiahparry.com/reference/st_knn.html) of sfdep package is used to determine the k (i.e. 6) nearest neighbours for given point geometry. The function [`st_kernel_weights()`](https://sfdep.josiahparry.com/reference/st_kernel_weights.html) is used to derive a weights list by using a kernel function. To compute LCLQ, the reference point data must be in either character or vector list. We create two vector lists - one of Functional and for Non-Functional water point and are called A and B respectively. The code [`local_colocation()`](https://sfdep.josiahparry.com/reference/local_colocation.html) os used to compute the LCLQ values for each water point. Before we can plot the LCLQ values their p-values, we need to join the output of `local_colocation()` to the stores sf data.frame.
