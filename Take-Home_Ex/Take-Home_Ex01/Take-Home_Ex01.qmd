...
Hence, we use `st_transform()` and not `st_set_crs()` as `st_set_crs()` assigns the EPSG code to the data frame. And we need to transform the data frame from geographic to projected coordinate system. We will be using crs=26392 (found from the [EPSG](https://epsg.io/?q=Nigeria%20kind%3APROJCRS) for Nigeria).
...
-   **H~0~:** The distribution of the Functional Water Points are randomly distributed

-   **H~1~:** The distribution of the Functional Water Points are not randomly distributed
...
-   **H~0~:** The distribution of the Non-Functional Water Points are randomly distributed

-   **H~1~:** The distribution of the Non-Functional Water Points are not randomly distributed
...
-   **H~0~:** The spatial distribution of Functional and Non-Functional water points are spatially independent

-   **H~1~:** The spatial distribution of Functional and Non-Functional water points are not spatially independent
...
1.  Professor Kam Tin Seong's [Hands-on Excercises](https://r4gdsa.netlify.app/chap04.html)

2.  [G - Function Point Patter Analysis](https://www.youtube.com/watch?v=npWXTbRNLtM&ab_channel=GeoDaSoftware)

3.  Selection of bandwidth type and adjustment side in kernel density estimation over inhomogeneous backgrounds ([Paper](https://www.tandfonline.com/doi/full/10.1080/13658810902950625))

4.  How IDW works ([Reference](https://desktop.arcgis.com/en/arcmap/10.3/tools/3d-analyst-toolbox/how-idw-works.htm#:~:text=Inverse%20distance%20weighted%20(IDW)%20interpolation,of%20a%20locationally%20dependent%20variable))
